...
export function InsightsListingToolbar({ categories }: Props) {
  ...
  const isAll = pathname === "/insights" || pathname === "/insights/";
  ...
  const slugMissingFromList =
    Boolean(activeSlug) &&
    !isAll &&
    !sorted.some((category) => category.slug === activeSlug);

  const selectId = "insights-category-select";
