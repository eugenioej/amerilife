...
import { CAREER_LEADERBOARD_CONFIG } from "@/lib/ideaxchange-career-leaderboard-data";

function scrollToTable(slug: string) {
  document.getElementById(`career-leaderboard-table-${slug}`)?.scrollIntoView({
    behavior: "smooth",
    block: "start",
  });
}

export function CareerLeaderboardQuickNav() {
  return (
    <nav
      className="border-b border-[var(--color-border)] bg-[#f7faf9]"
      aria-label="Career leaderboard table navigation"
    >
      <div className="mx-auto max-w-[var(--container-max)] px-[var(--container-padding-x)] py-8 md:py-10">
        ...
        <div className="mt-6 flex flex-col gap-8 md:gap-10">
          {CAREER_LEADERBOARD_CONFIG.map((section) => (
