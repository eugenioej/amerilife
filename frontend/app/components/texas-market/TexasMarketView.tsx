...
import {
  TEXAS_MARKET_OFFICES,
  TEXAS_MY_MAP_EMBED_SRC,
  type TexasMarketOffice,
} from "@/lib/texas-market-data";
import { telHrefPlusOne } from "@/lib/us-tel-href";

/** Outline CTA — map opens externally. */
...
/** Solid green — matches `Button` primary variant. */
...
type TexasMarketViewProps = {
  connectForm: GfFormData | null;
};

function OfficeCard({ office }: { office: TexasMarketOffice }) {
...
export function TexasMarketView({ connectForm }: TexasMarketViewProps) {
  return (
    <article className="bg-white">
      <div className="border-b border-[#dfe7ec] bg-[#f6faff]">
        <div className="mx-auto max-w-[var(--container-max)] px-[var(--container-padding-x)] py-3 sm:py-[0.9rem]">
          <SiteBreadcrumb items={[{ label: "Home", href: "/" }, { label: "Texas Market" }]} />
        </div>
      </div>
      ...
      <div className="w-full bg-[var(--color-brand-primary)] px-[var(--container-padding-x)] py-11 text-white sm:py-14">
        <div className="mx-auto max-w-[var(--container-max)] text-left">
          <h1 className="text-[1.6875rem] font-bold leading-tight tracking-tight sm:text-[2.125rem] lg:text-[2.625rem]">
            Find an Agent Texas Market
          </h1>
          <p className="mt-5 max-w-2xl text-lg font-semibold text-white/95 sm:text-xl">AmeriLife of Texas, LLC</p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[var(--container-max)] px-[var(--container-padding-x)] py-12 sm:py-14 lg:py-16">
        <div className="grid grid-cols-1 gap-8 sm:gap-10 md:grid-cols-2 lg:grid-cols-3">
          {TEXAS_MARKET_OFFICES.map((office) => (
      ...
      <section className="border-t border-[var(--color-border)] bg-[#eef5f4] px-[var(--container-padding-x)] py-12 sm:py-14">
        <div className="mx-auto max-w-[var(--container-max)]">
          <h2 className="mb-2 text-xl font-bold text-[var(--color-brand-dark)] sm:text-2xl">
            Texas career agency offices
          </h2>
          <p className="mb-6 max-w-3xl text-sm text-[var(--color-muted)] sm:text-[0.95rem]">
            Interactive map (same embedded layer as amerilife.com/texas).
          </p>

          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(280px,400px)] lg:items-start lg:gap-x-10 xl:gap-x-12">
            <div className="relative min-h-[380px] w-full overflow-hidden rounded-lg border border-[var(--color-border)] bg-[#dfe8e6] shadow-sm sm:min-h-[440px] lg:min-h-[500px]">
              <iframe
                title="AmeriLife Texas career agency offices map"
                src={TEXAS_MY_MAP_EMBED_SRC}
            ...
            <div className="rounded-lg border border-[var(--color-border)] bg-white p-6 shadow-sm sm:p-8">
              <h3 className="mb-6 text-lg font-bold text-[var(--color-brand-dark)] sm:text-xl">
                Find an agent
              </h3>
              {connectForm ? (
                <>
      ...
      <section className="border-t border-[var(--color-border)] bg-[#f8fafb] py-12 text-left sm:py-16">
        <div className="mx-auto max-w-[var(--container-max)] px-[var(--container-padding-x)] text-left">
          ...
          <div className="mt-8 max-w-4xl space-y-6 text-left text-base leading-[1.75] text-[var(--color-fg)]">
            <p>
